...
class ScanActivity : BaseActivity(), QRCodeView.Delegate {
    ...
    override fun initHeader() {
        super.initHeader()
        rightTv.text = getString(R.string.line_code)
...
//                    .forResult(REQUEST_CODE_CHOOSE)
    ...
    private fun change2BarCodeScan() {
        isBarCode = !isBarCode
        rightTv.text = if (isBarCode) getString(R.string.bar_code) else getString(R.string.line_code)

        if (isBarCode) {
//            mZXingView.setType(BarcodeType.ONE_DIMENSION, null);
            mZXingView.scanBoxView.isBarcode = true
            mZXingView.scanBoxView.rectWidth = DisplayUtils.dip2px(300F)
        } else {
//            mZXingView.setType(BarcodeType.TWO_DIMENSION, null);
            mZXingView.scanBoxView.isBarcode = false
            mZXingView.scanBoxView.rectWidth = DisplayUtils.dip2px(200F)
        }
    }
...
//        mZXingView.startCamera(Camera.CameraInfo.CAMERA_FACING_FRONT); // 打开前置摄像头开始预览，但是并未开始识别
    ...
    override fun onCameraAmbientBrightnessChanged(isDark: Boolean) {
        ...
        val ambientBrightnessTip = getString(R.string.scan_dark_tip)
        if (isDark) {
    ...
    companion object{
        fun getIntent(context: Context): Intent{
            return Intent(context, ScanActivity.javaClass)
        }
    }
