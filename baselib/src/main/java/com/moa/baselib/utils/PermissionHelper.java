...
public class PermissionHelper {
    ...
    public static boolean checkCameraPermissions(Fragment fragment, Activity context, int requestCode){
        return checkAndRequestPermissions(fragment, context, requestCode, new String[]{
                Manifest.permission.READ_EXTERNAL_STORAGE,// sdcard
                Manifest.permission.WRITE_EXTERNAL_STORAGE,
    ...
    public static boolean containsAndAcceptPermission(String[] reqPermissions, String hasPermission, int[] grands){
        if(reqPermissions != null && reqPermissions.length > 0
            ...
            if(index >= 0 && grands.length > index){
                return vertifiedPermission(grands[index]);
            }
        }
     ...
     * @return
     */
    public static boolean vertifiedPermission(int[] grantResults) {
        if (grantResults != null) {
            for (int result: grantResults){
                // 有一个权限没有获取到就返回false
                 if(!vertifiedPermission(result)){
                     return false;
                 }
            }
            // 全部权限都获取到
     ...
     * @return
     */
    public static boolean vertifiedPermission(int grantResult) {
